...
A new analytics layer that audits the actual alpha generated by sentiment signals, accessible at `/alpha` (link appears when Advanced mode is enabled on the main dashboard).

### Feature Attribution Logging

Every analysis run now records which news category drove each signal and which keyword terms matched:

- `event_type` column promoted from the Stage 2 LLM JSON blob to a dedicated queryable column on `decision_log_symbol`
- `keyword_attribution` JSON column records per-symbol hit counts for the top matched proxy terms, plus a source-level breakdown (e.g., `reuters_rss: 7`, `marketwatch: 3`)
- Stage 1 keyword matching now counts term hits per article per symbol during filtering — zero additional LLM calls

### Rolling Information Coefficient (IC)

New endpoint `GET /api/v1/alpha/ic` computes rolling Spearman rank correlation between `blended_confidence_score` and actual forward returns:

- Horizons: 4h, 1d, 3d, 1w (matched to existing `TradeSnapshot` data — no new collection needed; 1h excluded as too noisy for sentiment)
- Configurable rolling window (default 30 trades)
- Pure Python implementation — no scipy dependency
- Returns overall IC, rolling series, and scatter pairs per horizon

### Sensitivity / Perturbation Testing

New endpoint `POST /api/v1/alpha/perturbation` replays historical signals with the entry threshold nudged by ±N%:

- Shows how signal count and average return change at baseline, +N%, and −N% thresholds
- Reveals whether threshold is genuinely discriminating or accidentally curve-fit to noise
- Configurable nudge percentage (default ±10%)

### Alpha Analytics Dashboard (`/alpha`)

New frontend page with four panels:

- **IC summary chips** — one per horizon, click to switch the chart focus
- **Rolling IC line chart** — tracks whether predictive power is stable or decaying
- **Signal Confidence vs. Return scatter** — every signal plotted by confidence vs realized return, colored by event type
- **Event type attribution bar chart** — stacked LONG/SHORT/HOLD counts per category + top matched keywords
- **Perturbation comparison panel** — side-by-side baseline / nudge-up / nudge-down with per-horizon returns

### Schema Changes

Two new nullable columns added to `decision_log_symbol` (in `decision_log.db`):
- `event_type VARCHAR(32)` — promoted from `blue_team_structured_output` JSON
- `keyword_attribution JSON` — Stage 1 keyword hit counts and source breakdown

Migration is automatic on startup. Existing rows are unaffected (columns default to NULL).
